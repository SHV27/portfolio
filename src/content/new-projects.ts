...
export const JARVIS: Project = {
  ...
  mechanism: [
    {
      lead: 'Three layers, and each one only hands upward what it genuinely cannot do itself. The bottom layer has no AI in it at all, which is exactly why it is instant.',
      text: 'It works in three layers and each one only passes upward what it genuinely cannot handle. The reflex arc is pure code and answers in twelve and a half milliseconds at its very worst, under one at its median. Above it, a model writes a new skill exactly once. Above that, it just talks to you.',
      receipts: [{ label: 'measured · source private', proves: '84 commits. The architecture, the decision log and the audit scripts are all in it.' }],
    },
    {
      lead: 'It does not try to understand the name of the thing you asked for — it looks it up in a live list of everything actually on the machine. That is why “open youtube and play stand up video” stops trying to launch an app called “youtube and play stand up video”.',
      ...
      receipts: [{ label: 'measured · source private', proves: 'Catalogue lookup rather than regex capture — the specific fix for the defect class that broke the previous version.' }],
    },
    {
      lead: '“youtube kholo aur stand up comedy chalao” works exactly as well as the English, because the verb is allowed to sit anywhere in the sentence instead of being assumed to come first.',
      ...
      receipts: [{ label: 'measured · source private', proves: 'Hindi word order handled in the resolver itself.' }],
    },
    {
      lead: 'Ask for something it has never done and it writes itself a recipe — out of 43 pre-approved building blocks, never raw commands. It runs it, checks the thing really happened, and only keeps recipes that worked. Tomorrow the same request is plain code with no AI in it at all.',
      ...
      receipts: [{ label: 'measured · source private', proves: '43 primitives in a registry; a plan may only name registry entries, never a shell command.' }],
    },
    {
      lead: 'It can drive apps nobody wrote any support for, by reading the window the way a screen reader does and clicking things by their name. Word, Discord, VS Code, Settings — not one line of code written per app.',
      ...
      receipts: [{ label: 'measured · source private', proves: 'Accessibility-tree driving, with the window verified to be in front before any input is sent.' }],
    },
    {
      lead: 'A laptop that listens all the time hears its own speakers. So it takes whatever the machine is playing, feeds it back in as a reference, and subtracts it before anything downstream hears a thing.',
      ...
      receipts: [{ label: 'measured · source private', proves: 'Loopback echo cancellation plus a speaker-embedding gate that self-enrols and degrades to off with a visible badge.' }],
    },
  ],
  figures: [
    { value: '12.5', unit: ' ms', label: 'worst case to understand a command — median under 1 ms', receipts: [{ label: 'measured · source private', proves: 'Measured across 5,052 sentences generated from this machine\'s own catalogue.' }] },
    { value: '5,052', label: 'sentences swept, English and Hinglish, every one resolving correctly', receipts: [{ label: 'measured · source private', proves: 'An audit script generates them from the real catalogue; a test is the gate.' }] },
    { value: '43', label: 'verified building blocks a new skill may be composed from — and nothing else', receipts: [{ label: 'measured · source private', proves: 'A plan may only name registry primitives, never a shell. A learned skill cannot become a learned mistake.' }] },
    { value: '12.3', unit: ' dB', label: 'of its own speakers removed from the microphone, at 2.3 ms of CPU per 100 ms of audio', receipts: [{ label: 'measured · source private', proves: 'WASAPI loopback as the far-end reference into echo cancellation.' }] },
    { value: '23s → 0.0125s', label: 'the gap it closes: a cold model turn versus the reflex arc', receipts: [{ label: 'measured · source private', proves: 'Measured on the same laptop — the reason a model is never in the hot path.' }] },
    { value: '0', label: 'API keys, subscriptions or cloud calls needed to use it', receipts: [{ label: 'measured · source private', proves: 'Single user, runs on the machine, works with no key at all.' }] },
  ],
  restraint: {
    ...
    receipts: [{ label: 'measured · source private', proves: 'The catalogue sweep and the six defect classes it caught on its first run.' }],
  },
...
export const RECKONER: Project = {
  ...
  mechanism: [
    {
      lead: 'Before it explains anything at all, it asks whether your question was even fair. One period in twenty-five fails — and it is the one everybody would have put in the board pack.',
    ...
    {
      lead: 'Then it hands you the question you should have asked instead: the same month, one year apart, both of them complete.',
    ...
    {
      lead: 'Even “roughly flat” turns out to be a disguise. Flat here means two much larger forces pulling against each other and very nearly cancelling.',
      text: 'And "plus 2.7%" is not the finding either — it is the disguise. Underneath it, £484k of brand-new products is being swallowed by £245k of lost volume, £138k of mix and £135k of discontinued lines, with £73k of price the only other thing pushing back. Five named pieces, and they reconcile back to the totals exactly.',
    ...
    {
      lead: 'The “is your data any good” check runs FIRST, not in a footnote at the bottom. If your file is wrong this tool will explain a wrong number beautifully, and you need to know that before you read the answer, not after.',
    ...
    {
      lead: 'A million rows, and not one of them leaves your laptop. No server, no upload, no account — which is the only reason anybody would ever point it at a real ledger.',
  ...
  shots: [
    ...
    { file: 'reckoner/act-3.png', alt: 'A waterfall chart decomposing a revenue change into volume, price, mix and interaction, new products and discontinued lines.', caption: 'Act Three. The honest comparison looks flat at +2.7% — until the waterfall opens it up: £484k of new products, £245k of lost volume, £138k of mix, £135k of lines discontinued, £73k of price. A company holding still because two much larger forces pull against each other is not the same company as one that is genuinely flat.' },

...
export const SEHAT: Project = {
  ...
  mechanism: [
    ...
    {
      lead: 'If it says eighty per cent, it has to be right about eight times in ten. Every probability is put through a correction that forces it to match how often it is really right, before a human ever sees it.',
...
export const GLOAMING: Project = {
  ...
  restraint: {
    text: 'My skill gap came out flat on the first measurement — a good player was winning no more often than a greedy one, which meant the decisions did not matter. Rather than shipping and hoping, I changed the design until they did: the torch went from eight to seven, and stepping on frayed ground went from costing one to costing two. That is what turned reading the monster’s telegraph into an actual skill. I tuned it until skill provably paid, and left the proof in the repository.',
